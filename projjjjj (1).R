...
#Calculating volatilities for each financial crisis
...
kurtosis(crisis_2008)
kurtosis(crisis_2020)
kurtosis(crisis_2022)
...
save(GARCH1000, file = "GARCH1000GS.RData")
save(GARCH500, file = "GARCH500GS.RData")
save(tGARCH1000, file = "tGARCH1000GS.RData")
save(HS500, file = "HS500GS.RData")
save(HS1000, file = "HS1000GS.RData")
save(EWMA, file = "EWMAGS.RData")

load("GARCH1000GS.RData")
load("GARCH500GS.RData")
load("tGARCH1000GS.RData")
load("HS500GS.RData")
load("HS1000GS.RData")
load("EWMAGS.RData")
...
# Performing significance tests for backtesting

T_2008 <- length(crisis_2008)
p = 0.05
WE = 1000
...
m <- cbind("HS500", "HS1000", "EWMA", "GARCH500", "GARCH1000", "tGARCH1000")

for (i in 1:6){
  ber = bern_test(p,Violations[,i])
  cat(i,m[i],"Bernoulli",ber,1 - pchisq(ber,1),"\n")
}

for (i in 1:6){
  ber = bern_test(p,Violations_crisis_2008[,i])
  cat(i,m[i],"Bernoulli",ber,1 - pchisq(ber,1),"\n")
}

for (i in 1:6){
  ber = bern_test(p,Violations_crisis_2020[,i])
  cat(i,m[i],"Bernoulli",ber,1 - pchisq(ber,1),"\n")
}

for (i in 1:6){
  ber = bern_test(p,Violations_crisis_2022[,i])
  cat(i,m[i],"Bernoulli",ber,1 - pchisq(ber,1),"\n")
}
...
for (i in 1:6){
  ind = ind_test(Violations_crisis_2008[,i])
  cat(i,m[i],"independence", ind,1 - pchisq(ind,1),"\n")
}


for (i in 1:6){
  ind = ind_test(Violations_crisis_2020[,i])
  cat(i,m[i],"independence", ind,1 - pchisq(ind,1),"\n")
}


for (i in 1:6){
  ind = ind_test(Violations_crisis_2022[,i])
  cat(i,m[i],"independence", ind,1 - pchisq(ind,1),"\n")
}